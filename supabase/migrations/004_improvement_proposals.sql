...
select * from (values
  ...
  ('Google カレンダー連携（任意の上乗せ・審査通過後に発表）',
   E'【決まった方針】\n連携は必須にしない。連携しなくても Eizo Maps だけで完結する。Google を使っている人が連携すると、そこからさらに便利になる、という上乗せの位置づけ。\n発表は Google の審査が通ってから行う。審査前に世に出さないので、警告画面も100人の上限もユーザーの目に触れない。\n\n【連携すると増えること（案）】\n・Eizo Maps に入れた予定が Google カレンダーにそのまま入る\n・Google カレンダー側の予定も踏まえて「埋まっている日」を判定できる\n・日程調整の候補出しの精度が上がる\n\n【段取り】\n1. 実装する（審査でデモ動画の提出が要るため、動くものが先に必要）\n2. Testing 状態で自分たちのアカウントを含む招待者だけで動作確認する\n3. Production に切り替えて審査に出す（通常3〜5営業日・無料）\n4. 通ったら発表する\n\n【実装時の注意】\n・Testing のあいだは refresh token が7日で切れる。裏の同期は7日ごとに止まるので、その期間の検証では再連携が要る。Production に上がれば解消する。\n・1つの Google アカウント × 1クライアントにつき refresh token は100個まで。超えると古いものから無効になる。\n・トークンはブラウザに置けない。保管と定期同期のために Edge Function が要る。\n・審査には、ドメイン所有の確認（Search Console）、プライバシーポリシー、OAuth 同意画面の記載、スコープが必要な理由の説明、操作のデモ動画が要る。ドメインとプライバシーポリシーは用意済み。\n\n【保留】\nApple／Outlook の人向けの購読URL方式は、必須ではなくなった。予定のテーブルは共通なので、必要になった時点で足せる。',
   'schedule', 'user_voice', 'high', 'high',
   '無料。審査に3〜5営業日。Edge Function は無料枠内'),
