...
4. 続けて `supabase/migrations/` の中を**番号順に**すべて貼り付けて **Run**。
   - `001_stock_pages.sql` … ストックページ（パスワード保護のポートフォリオ）
   - 既に稼働中のプロジェクトに機能を足すときも、このファイルを実行するだけです。
     既存のテーブルには触りません。
5. 続けて `supabase/verify.sql` を貼って **Run**。
6. Google カレンダー連携を使う場合のみ、`docs/GOOGLE_CALENDAR.md` の手順を実施する
   （Google Cloud の設定と Edge Function のデプロイ。連携は任意機能なので、
   ここを飛ばしても Eizo Maps は問題なく動きます）。
